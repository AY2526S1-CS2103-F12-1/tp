...
1. When `Logic` is called upon to execute a command, it is passed to an `HenriParser` object which in turn creates a parser that matches the command (e.g., `DeleteCommandParser`) and uses it to parse the command.
...
* When called upon to parse a user command, the `HenriParser` class creates an `XYZCommandParser` (`XYZ` is a placeholder for the specific command name e.g., `AddCommandParser`) which uses the other classes shown above to parse the user command and create a `XYZCommand` object (e.g., `AddCommand`) which the `HenriParser` returns back as a `Command` object.
...
* stores Henri data i.e., all `Person` objects (which are contained in a `UniquePersonList` object).
...
**Note:** An alternative (arguably, a more OOP) model is given below. It has a `Tag` list in the `Henri`, which `Person` references. This allows `Henri` to only require one `Tag` object per unique tag, instead of each `Person` needing their own `Tag` objects.<br>
...
* can save both Henri data and user preference data in JSON format, and read them back into corresponding objects.
* inherits from both `HenriStorage` and `UserPrefStorage`, which means it can be treated as either one (if only the functionality of only one is needed).
* depends on some classes in the `Model` component (because the `Storage` component's job is to save/retrieve objects that belong to the `Model`)

### Common classes

Classes used by multiple components are in the `seedu.henri.commons` package.
...
The proposed undo/redo mechanism is facilitated by `VersionedHenri`. It extends `Henri` with an undo/redo history, stored internally as an `henriStateList` and `currentStatePointer`. Additionally, it implements the following operations:

* `VersionedHenri#commit()` — Saves the current Henri state in its history.
* `VersionedHenri#undo()` — Restores the previous Henri state from its history.
* `VersionedHenri#redo()` — Restores a previously undone Henri state from its history.

These operations are exposed in the `Model` interface as `Model#commitHenri()`, `Model#undoHenri()` and `Model#redoHenri()` respectively.

Given below is an example usage scenario and how the undo/redo mechanism behaves at each step.

Step 1. The user launches the application for the first time. The `VersionedHenri` will be initialized with the initial Henri state, and the `currentStatePointer` pointing to that single Henri state.

<puml src="diagrams/UndoRedoState0.puml" alt="UndoRedoState0" />

Step 2. The user executes `delete E1005` command to delete the 5th person in the Henri. The `delete` command calls `Model#commitHenri()`, causing the modified state of the Henri after the `delete E1005` command executes to be saved in the `henriStateList`, and the `currentStatePointer` is shifted to the newly inserted Henri state.

<puml src="diagrams/UndoRedoState1.puml" alt="UndoRedoState1" />

Step 3. The user executes `add n/David …​` to add a new person. The `add` command also calls `Model#commitHenri()`, causing another modified Henri state to be saved into the `henriStateList`.

<puml src="diagrams/UndoRedoState2.puml" alt="UndoRedoState2" />

<box type="info" seamless>

**Note:** If a command fails its execution, it will not call `Model#commitHenri()`, so the Henri state will not be saved into the `henriStateList`.

</box>

Step 4. The user now decides that adding the person was a mistake, and decides to undo that action by executing the `undo` command. The `undo` command will call `Model#undoHenri()`, which will shift the `currentStatePointer` once to the left, pointing it to the previous Henri state, and restores the Henri to that state.
...
**Note:** If the `currentStatePointer` is at index 0, pointing to the initial Henri state, then there are no previous Henri states to restore. The `undo` command uses `Model#canUndoHenri()` to check if this is the case. If so, it will return an error to the user rather
...
The `redo` command does the opposite — it calls `Model#redoHenri()`, which shifts the `currentStatePointer` once to the right, pointing to the previously undone state, and restores the Henri to that state.

<box type="info" seamless>

**Note:** If the `currentStatePointer` is at index `henriStateList.size() - 1`, pointing to the latest Henri state, then there are no undone Henri states to restore. The `redo` command uses `Model#canRedoHenri()` to check if this is the case. If so, it will return an error to the user rather than attempting to perform the redo.

</box>

Step 5. The user then decides to execute the command `list`. Commands that do not modify the Henri, such as `list`, will usually not call `Model#commitHenri()`, `Model#undoHenri()` or `Model#redoHenri()`. Thus, the `henriStateList` remains unchanged.

<puml src="diagrams/UndoRedoState4.puml" alt="UndoRedoState4" />

Step 6. The user executes `clear`, which calls `Model#commitHenri()`. Since the `currentStatePointer` is not pointing at the end of the `henriStateList`, all Henri states after the `currentStatePointer` will be purged. Reason: It no longer makes sense to redo the `add n/David …​` command. This is the behavior that most modern desktop applications follow.
...
* **Alternative 1 (current choice):** Saves the entire Henri.
...
| `* * *`  | HR administrator | add people’s contact to my Henri                                                  | add them when they join the company                                |
| `* * *`  | HR administrator | delete people’s contact to my Henri                                               | remove them when they leave the company                            |
...
(For all use cases below, the **System** is the `Henri` and the **Actor** is the `user`, unless specified otherwise)
